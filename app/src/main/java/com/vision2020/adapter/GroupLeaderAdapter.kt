...
class GroupLeaderAdapter(
    var mData: ArrayList<GroupLeader>,
    var mClick: ItemCallBack.IndexClick
) : RecyclerView.Adapter<GroupLeaderAdapter.ViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.item_options, parent, false)
        return ViewHolder(v)
    }
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.txtOption.text = mData[position].StudentName
